/*************************************************************************
 * Lab 6 - Test 3 - Hunsung Lee & Jonghoon Choi
 * This test is the example on Lecture 14, Slide 12
 * The PIP causes Chained Blocking
 ************************************************************************/
...
realtime_t t_pRT4start = {0, 150};

/* Process deadline */
realtime_t t_pRT1dead = {1, 0};
realtime_t t_pRT2dead = {0, 750};
realtime_t t_pRT3dead = {0, 600};
realtime_t t_pRT4dead = {0, 450};

/* Process period */
realtime_t t_pRT1period = {2, 0};
...
void p4(void) {
	...
	l_lock(&l);
	PIT->CHANNEL[0].TCTRL = 0x1;
	debug_printf("P4 - Critical Section (LOCK 1) \r\n");
	PIT->CHANNEL[0].TCTRL = 0x3;
	for(int x = 0; x < 5; x++){
		PIT->CHANNEL[0].TCTRL = 0x1;
		debug_printf("P4 - CS Cycle (LOCK 1) %d \r\n", x);
		LEDBlue_Toggle();
		LEDRed_Toggle();		
		mediumDelay();
		PIT->CHANNEL[0].TCTRL = 0x3;
	...
	PIT->CHANNEL[0].TCTRL = 0x1;
	debug_printf("P4 - Critical Section (LOCK 2) \r\n");
	PIT->CHANNEL[0].TCTRL = 0x3;
	for(int x = 0; x < 5; x++){
		PIT->CHANNEL[0].TCTRL = 0x1;
		debug_printf("P4 - CS Cycle (LOCK 2) %d \r\n", x);
		LEDBlue_Toggle();
		LEDRed_Toggle();		
		mediumDelay();
		PIT->CHANNEL[0].TCTRL = 0x3;
	...
	l_lock(&l3);
	PIT->CHANNEL[0].TCTRL = 0x1;
	debug_printf("P4 - Critical Section (LOCK 3) \r\n");
	PIT->CHANNEL[0].TCTRL = 0x3;
	for(int x = 0; x < 5; x++){
		PIT->CHANNEL[0].TCTRL = 0x1;
		debug_printf("P4 - CS Cycle (LOCK 3) %d \r\n", x);
		LEDBlue_Toggle();
		LEDRed_Toggle();		
		mediumDelay();
		PIT->CHANNEL[0].TCTRL = 0x3;
