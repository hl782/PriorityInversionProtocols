/*************************************************************************
 * Lab 6 - Test 1 - Hunsung Lee & Jonghoon Choi
 * This test is the example on Lecture 14, Slide 9
************************************************************************/
...
realtime_t t_pRT2start = {0, 100};
realtime_t t_pRT3start = {0, 50};
/* Process deadline */
realtime_t t_pRT1dead = {1, 0};
realtime_t t_pRT2dead = {0, 200};
realtime_t t_pRT3dead = {0, 150};
/* Process period */
...
void p2(void) {
	PIT->CHANNEL[0].TCTRL = 0x1;
	debug_printf("P2 - NonCritical Section \r\n");
	PIT->CHANNEL[0].TCTRL = 0x3;
	
	for(int x = 0; x < 2; x++){
		PIT->CHANNEL[0].TCTRL = 0x1;
		debug_printf("P2 - NCS Cycle %d \r\n", x);
		LEDRed_Toggle();
		mediumDelay();
		PIT->CHANNEL[0].TCTRL = 0x3;
	}
